...
// shoudl I make this abstract and
class Question {

public:
    Question(string que, string corr_ans):question(que),correct_answer(corr_ans) {};
...
public:
    ...
    MultipleChoiseQuestion(string que, string corr_ans, vector<string> ans):Question(que, corr_ans),answers(ans){};
...
int main() {
    ...
    list.push_back(Question("What is always coming but never arrives? ","tomorrow"));
    ans = { "usa", "australia", "south africa", "new zeland" };
    list.push_back(MultipleChoiseQuestion("Which country do cities of Perth, Adelade & Brisbane belong to?", "australia", ans));
    cout << "There are "<< list.size() << " question, some with multiple choise where you have to select the correct one" << endl;
